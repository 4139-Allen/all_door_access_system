...
**Key Files:**
- `backend/database/migrations/alembic.ini` - Alembic configuration
- `backend/database/migrations/scripts/env.py` - Environment setup (imports project models and config)
- `backend/database/migrations/scripts/versions/` - Migration scripts directory
- `backend/database/migrations/manage_db.py` - Management script for common operations
...
- Migrations are stored in `backend/database/migrations/scripts/versions/`
- Schema management is **pure Alembic** — `create_all()` is no longer used. `init_database()` only ensures the database exists, then runs `alembic upgrade head` (idempotent)
- New/fresh databases get all tables from the initial baseline migration (`46c280a24a6f`); existing databases get incremental migrations
- **Every schema change must go through a migration**: edit the model → `manage_db.py create -m "desc"` → review → `manage_db.py upgrade`. Editing only the model has no effect on an existing database
- `upgrade` auto-fixes a stale `alembic_version` (leftover from pre-baseline migrations) by stamping to head first, so it's safe to run repeatedly
